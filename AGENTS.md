...
4. **雙軌測試版發布機制 (Dual-track Staging Release Protocol)**：
   * **正式版 (Production)**：根目錄 `index.html`，對應網址為 `https://davislinyd.github.io/LagCheck/`。
   * **測試版 (Staging / Beta)**：根目錄 `test.html`，對應網址為 `https://davislinyd.github.io/LagCheck/test.html`。
   * **標準發布與驗證流程**：
